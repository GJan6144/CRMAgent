...
默认策略：
  查询类 → allow；CRM / 知识库 / 文档的**写入**类 → approval；
  删除类，以及**文件写入 / 修改 / 执行命令**类 → deny
  （本项目是 CRM 业务助手：Agent 不得改动任何文件、不得执行本地命令）。
"""
...
TOOL_CATALOG: list[dict] = [
    ...
    # ---------------- 文件系统与 Shell ----------------
    # ⚠️⚠️ 写文件 / 改文件 / 执行命令一律**默认禁止**：本项目是 CRM 业务助手，
    #    不是编码助手 —— Agent 不得改动任何文件（源码 / 配置 / 数据都算），
    #    也不得执行本地命令。这是「禁止改源码」在策略层的硬拦截，
    #    模型即使尝试调用也会被 FsApprovalMiddleware 拦下并推送「禁止」提示。
    ...
    _t("write_file", "写入文件", "文件系统与 Shell",
       "写入 / 修改文件（禁止：业务助手不得改动文件）", POLICY_DENY),
    _t("edit_file", "修改文件", "文件系统与 Shell",
       "按片段修改已有文件（禁止：业务助手不得改动文件）", POLICY_DENY),
    _t("delete", "删除文件", "文件系统与 Shell", "删除文件（默认禁止）", POLICY_DENY),
    _t("execute", "执行命令", "文件系统与 Shell",
       "执行本地 Shell 命令（禁止：业务助手不得执行命令）", POLICY_DENY),
